...
const NovoEspecificadorDialog = ({ open, onOpenChange, onSave, especificadorToEdit, allEspecificadores, allUsers }) => {
    ...
    const initialFormData = { 
        nome_empresa: '', email: '', telefone1: '', cpf: '', is_master: false, master_id: 'none', indicado_por: 'none' // Default to 'none'
    };
    ...
    const handleSelectChange = (field, value) => {
        setFormData(prev => ({ ...prev, [field]: value === 'none' ? null : value })); // Handle 'none'
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { error: especError, data: especData } = await supabase
                ...
                .upsert({ ...formData, master_id: formData.master_id === 'none' ? null : formData.master_id, indicado_por: formData.indicado_por === 'none' ? null : formData.indicado_por }, { onConflict: 'id' }) // Handle 'none'
                .select()
                .single();
                ...
                <form onSubmit={handleSubmit} className="space-y-4 py-4 max-h-[80vh] overflow-y-auto pr-4">
                    ...
                    <div className="border-t pt-4 mt-4 space-y-4">
                        ...
                        <div className="space-y-2"><Label>Master (Líder da Rede)</Label>
                            <Select value={formData.master_id} onValueChange={v => handleSelectChange('master_id', v)}>
                        ...
                        <div className="space-y-2"><Label>Indicado Por (Usuário do Sistema)</Label>
                            <Select value={formData.indicado_por} onValueChange={v => handleSelectChange('indicado_por', v)}>
                                <SelectTrigger><SelectValue placeholder="Nenhum" /></SelectTrigger>
