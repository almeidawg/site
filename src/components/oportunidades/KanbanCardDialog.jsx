...
const KanbanCardDialog = ({ card, open, onOpenChange, onUpdate }) => {
    ...
    const fetchCardData = useCallback(async () => {
        ...
        if (cardData.responsavel_id) {
            const { data: userData, error: userError } = await supabase
                ...
                .eq('user_id', cardData.responsavel_id)
                .single();
    ...
    const handleSaveChanges = async () => {
        ...
        if (error) {
            toast({ title: 'Erro ao salvar alteracoes', description: error.message, variant: 'destructive' });
        } else {
    ...
    const handleAddComment = async () => {
        ...
        const comment = {
            id: Date.now(),
            text: newComment,
            author: currentUserProfile?.nome || 'Usuario',
                ...
                {loading || !cardDetails ? (
                    ...
                    <>
                        <DialogHeader>
                            ...
                            />
                            <DialogDescription>
                                Cliente: {cardDetails.cliente_nome || 'Nao especificado'}
                            </DialogDescription>
                            {responsibleUser && (
                                <div className="flex items-center gap-2 text-sm text-muted-foreground pt-2">
                                    <User className="h-4 w-4"/>
                                    <span>Responsavel: {responsibleUser.nome}</span>
                                </div>
                            )}
                            {cardDetails.servicos_contratados && cardDetails.servicos_contratados.length > 0 && (
                                <div className="text-xs text-muted-foreground pt-1">
                                  Servicos: {cardDetails.servicos_contratados.join(' / ')}
                                </div>
                            )}
                        </DialogHeader>
                        <div className="flex-grow overflow-y-auto pr-4 space-y-6">
                            ...
                            <section>
                                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2"><MessageSquare className="h-5 w-5"/> Comentarios</h3>
                                            ...
                                            <div className="flex-grow">
                                                <div className="flex items-baseline gap-2">
                                                    ...
                                                    <p className="text-xs text-muted-foreground">
                                                        {format(new Date(comment.created_at), "dd/MM/yy 'as' HH:mm", { locale: ptBR })}
                                                    </p>
                                                </div>
                                ...
                                <div className="mt-4">
                                    <Textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Adicionar um comentario..."/>
                                    <Button onClick={handleAddComment} className="mt-2">Enviar Comentario</Button>
                                </div>
                            </section>
