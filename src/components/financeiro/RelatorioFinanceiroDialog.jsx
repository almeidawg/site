...
const RelatorioFinanceiroDialog = ({ open, onOpenChange }) => {
    ...
    const handleFilterChange = (field, value) => {
        let finalValue = value === 'todos' || value === 'none' ? null : value; // Handle 'none'
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            const reportPath = `relatorios/financeiro-${Date.now()}.pdf`;

            const { data: functionData, error: functionError } = await supabase.functions.invoke('pdf-generate', {
                body: { html: reportHtml, path: reportPath },
            });
            
            if (functionError) throw new Error(functionError.message);
            if (functionData.error) throw new Error(functionData.error);
            
            window.open(functionData.url, '_blank');
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                    ...
                    <div className="space-y-2"><Label>Tipo</Label><Select value={filters.p_tipo} onValueChange={v => handleFilterChange('p_tipo', v)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="todos">Todos</SelectItem><SelectItem value="income">Receita</SelectItem><SelectItem value="expense">Despesa</SelectItem></SelectContent></Select></div>
                    <div className="space-y-2"><Label>Projeto (Centro de Custo)</Label><Autocomplete table="obras" displayColumn="nome" value={selectedItems.p_centro_custo} onChange={(item) => handleAutocompleteChange('p_centro_custo', item)} placeholder="Todos"/></div>
