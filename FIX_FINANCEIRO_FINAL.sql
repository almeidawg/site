-- =============================================
-- SCRIPT FINAL DE CORREÇÃO DO MÓDULO FINANCEIRO
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'obras') THEN
    IF NOT EXISTS (
      ...
      WHERE table_schema = 'public' AND table_name = 'obras' AND column_name = 'nome'
    ) THEN
    ...
    ELSE
      RAISE NOTICE '✓ obras.nome já existe';
    END IF;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entities') THEN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'entities' AND column_name = 'nome_razao_social'
    ) THEN
    ...
    ELSE
      RAISE NOTICE '✓ entities.nome_razao_social já existe';
    END IF;
  END IF;
END $$;
...
-- PASSO 7: Criar tabelas com dependências
...
SELECT '=== COLUNAS CRIADAS ===' as resultado;
