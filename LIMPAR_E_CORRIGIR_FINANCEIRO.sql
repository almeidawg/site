-- =============================================
-- SCRIPT DE LIMPEZA E CORREÇÃO DO MÓDULO FINANCEIRO
-- Execute este script diretamente no Supabase Dashboard
-- =============================================
-- Este script REMOVE tabelas problemáticas e recria corretamente
-- =============================================

-- PASSO 1: LIMPAR TABELAS PROBLEMÁTICAS (se existirem)
DROP TABLE IF EXISTS public.lancamentos_financeiros CASCADE;

-- PASSO 2: Adicionar coluna 'nome' na tabela 'obras' (alias para 'titulo')
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'obras') THEN
    IF NOT EXISTS (
      ...
      RAISE NOTICE '✓ Coluna obras.nome criada';
    ELSE
      RAISE NOTICE '✓ Coluna obras.nome já existe';
    END IF;
  ELSE
    RAISE NOTICE '⚠ Tabela obras não existe';
  END IF;
END $$;
  ...
  IF table_exists THEN
    ...
    IF NOT col_exists THEN
      IF EXISTS (
        ...
        RAISE NOTICE '✓ Coluna entities.nome_razao_social criada (alias de name)';
      ELSIF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public'
        AND table_name = 'entities'
        ...
        RAISE NOTICE '✓ Coluna entities.nome_razao_social criada (alias de nome)';
      ELSE
        ALTER TABLE public.entities ADD COLUMN nome_razao_social TEXT;
        RAISE NOTICE '✓ Coluna entities.nome_razao_social criada';
      END IF;
    ...
    END IF;
  ELSE
    RAISE NOTICE '⚠ Tabela entities não existe';
  END IF;
END $$;

-- PASSO 4: Criar tabelas independentes primeiro

-- 4.1: categorias_custo
...
-- PASSO 5: Criar tabelas com dependências
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠ Cobrancas: dependências não encontradas (obras ou entities)';
  END IF;
END $$;

-- 5.2: solicitacoes_pagamento
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'obras') THEN
  ...
  ELSE
    RAISE NOTICE '⚠ Solicitacoes: tabela obras não encontrada';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠ Comissoes: dependências não encontradas (profiles ou contratos)';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠ Reembolsos: tabela entities não encontrada';
  END IF;
END $$;
...
SELECT '✓ ' || table_name || '.' || column_name as coluna_criada
