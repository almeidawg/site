...
export const useCatalog = () => {
  ...
  useEffect(() => {
    fetchItems();
    const channel = supabase.channel('catalog_items_realtime_channel')
  ...
  const addItem = async (itemData) => {
    const { error } = await supabase.from('catalog_items').insert([itemData]);
    if (error) {
  ...
  const deleteItem = async (id) => {
    ...
    const { data: projectItems, error: checkError } = await supabase
        .from('project_items')
        .select('id')
  ...
  const bulkAddItems = async (itemsData) => {
    const { error } = await supabase.from('catalog_items').insert(itemsData);
    if (error) {
