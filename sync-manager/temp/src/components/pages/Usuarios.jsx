import React, { useState, useEffect, useCallback } from 'react';
    ...
    const Usuarios = () => {
        ...
        const { invoke: listUsers, loading: loadingUsers } = useSupabaseFunction('admin-list-users');
        const { invoke: sendInvite, loading: isInviting } = useSupabaseFunction('users-invite');
        ...
        const fetchUsers = useCallback(async () => {
            ...
            listUsers({
                onSuccess: (data) => {
                    const authUsersMap = data.users.reduce((acc, u) => ({ ...acc, [u.id]: u.email }), {});
                    const combined = profiles.map(p => ({
                        ...p,
                        email: authUsersMap[p.user_id] || 'N/A'
                    }));
        ...
        const handleSendInvite = async () => {
          ...
          await sendInvite({
            ...
            onError: (msg) => toast({ title: 'Erro ao enviar convite', description: msg, variant: 'destructive' })
          })
        };
                        ...
                        {usuarios.map((user, index) => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className={`px-3 py-1 text-xs font-medium rounded-full ${user.ativo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
