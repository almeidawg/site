    ...
    const NovaPropostaDialog = ({ open, onOpenChange, onSuccess, propostaToEdit, setPropostaToEdit }) => {
      ...
      useEffect(() => {
        const fetchInitialData = async () => {
            ...
            const { data: priceListData, error: priceListError } = await supabase.from('pricelist').select('*').order('description');
      ...
      const handleAddItem = (itemId) => {
        ...
        const newItem = {
            ...
            unidade: item.unit,
            quantidade: 1,
            valor_unitario: item.valor_venda_calculado,
            valor_total: item.valor_venda_calculado,
        };
                ...
                <div className="flex justify-between items-center">
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button type="button" variant="outline" size="sm" disabled={!formData.cliente_id || purchaseOrders.length === 0}>
                     ...
                     <Select onValueChange={handleAddItem}>
                        ...
                        <SelectContent>
                            {priceListItems.map(p => (
                                <SelectItem key={p.id} value={p.id}>{p.description} - {parseBRLToNumber(p.valor_venda_calculado).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>
