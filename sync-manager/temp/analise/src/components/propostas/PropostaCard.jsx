import React from 'react';
    ...
    import { FileText, Calendar, DollarSign, CheckCircle, XCircle, ShieldQuestion, FileSignature, Loader2 } from 'lucide-react';
    import { Button } from '@/components/ui/button';
    import { useAuth } from '@/contexts/SupabaseAuthContext';
    import ActionIcons from '@/components/shared/ActionIcons';

    const PropostaCard = ({ proposta, index, onApprove, onReject, onGerarContrato, onEdit, onGeneratePdf, onShareWhats, isGeneratingPdf, onDelete }) => {
        ...
        >
          ...
          {canApprove && proposta.status === 'aprovacao_pendente' && (
              <div className="mt-4 pt-4 border-t flex gap-2">
                  <Button variant="outline" size="sm" className="w-full text-red-600 border-red-300 hover:bg-red-50 hover:text-red-700" onClick={() => onReject(proposta.id)}>
                      <XCircle size={16} className="mr-2" />
                      Rejeitar
                  </Button>
                  <Button variant="outline" size="sm" className="w-full text-emerald-600 border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700" onClick={() => onApprove(proposta.id)}>
