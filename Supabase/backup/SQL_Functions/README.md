...
├── 02-security/          # RLS e permissões (futuro)
│   └── (vazio - criar quando necessário)
├── 03-validation/        # Validação de dados
│   ├── 01_cpf_cnpj_validation.sql
│   └── 02_format_br.sql
└── 04-business/          # Lógica de negócio (futuro)
    └── (vazio - criar quando necessário)
```
...
```bash
# 1. Abrir arquivo SQL
code Supabase/backup/SQL_Functions/01-triggers/01_handle_updated_at.sql
...
1. Decidir categoria (triggers, security, validation, business)
2. Criar arquivo com numeração: `0X_nome_descritivo.sql`
3. Usar template:

```sql
-- =============================================
-- CATEGORIA: Nome da Function
-- Descrição: O que faz
-- Uso: Quando usar
-- =============================================

DROP FUNCTION IF EXISTS nome_function() CASCADE;
...
COMMENT ON FUNCTION nome_function IS 'Descrição curta';
...
3. 🟡 Avaliar se precisa validações BR (CPF/CNPJ)
4. 🟡 Avaliar se precisa formatações BR (Phone/CEP)
5. 📝 Documentar qualquer nova function aqui
