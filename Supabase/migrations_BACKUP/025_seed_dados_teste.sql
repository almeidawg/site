...
INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2025-003', id, 'Obra Comercial Shopping', 'em_andamento', 850000.00, 45
FROM entities WHERE nome = 'Construtora ABC Ltda'
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2025-004', id, 'Reforma Escritório', 'em_andamento', 95000.00, 60
FROM entities WHERE nome = 'João Silva Construções'
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2025-005', id, 'Ampliação Galpão Industrial', 'em_andamento', 420000.00, 30
FROM entities WHERE nome = 'Maria Santos Arquitetura'
ON CONFLICT (codigo) DO NOTHING;

-- Obras CONCLUÍDAS (2)
INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2024-012', id, 'Casa de Campo Itatiba', 'concluida', 280000.00, 100
FROM entities WHERE nome = 'Construtora ABC Ltda'
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2024-015', id, 'Restaurante Boulevard', 'concluida', 160000.00, 100
FROM entities WHERE nome = 'João Silva Construções'
ON CONFLICT (codigo) DO NOTHING;

-- Obras PAUSADAS (1)
INSERT INTO obras (codigo, cliente_id, titulo, status, valor_orcado, progresso)
SELECT 'OBR-2025-006', id, 'Condomínio Residencial (Fase 2)', 'pausada', 1200000.00, 25
FROM entities WHERE nome = 'Construtora ABC Ltda'
ON CONFLICT (codigo) DO NOTHING;
