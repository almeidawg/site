...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const { error } = await signUp(formData.email, formData.password, {
            data: {
                nome: formData.name,
                perfil: 'pendente', // Perfil inicial até aprovação
            }
        });
        ...
        if (!error) {
            ...
            toast({
                ...
                description: 'Confirme seu e-mail para continuar. O administrador irá aprovar seu acesso em breve.',
            });
        } else {
             toast({
            ...
            <div className="min-h-screen w-full flex items-center justify-center bg-gray-100">
                <div className="w-full max-w-md mx-auto bg-white rounded-2xl shadow-xl p-8 text-center">
                    ...
                    <p className="mt-2 text-gray-600">
                        Enviamos um link de confirmação para o seu e-mail. Após a confirmação, um administrador irá aprovar seu cadastro.
                    </p>
                    ...
                    <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                        <div className="space-y-4 rounded-md shadow-sm">
                            <div>
                                ...
                                <Input id="name" type="text" required onChange={(e) => setFormData({...formData, name: e.target.value})} />
                            </div>
                            <div>
                                <Label htmlFor="email-address">Email</Label>
                                <Input id="email-address" type="email" required onChange={(e) => setFormData({...formData, email: e.target.value})} />
                            </div>
                            <div>
                                <Label htmlFor="password">Senha</Label>
                                <Input id="password" type="password" required minLength="6" onChange={(e) => setFormData({...formData, password: e.target.value})} />
                            </div>
                        </div>
