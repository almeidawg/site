...
const Dashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Simplificado: tabelas transacoes/solicitacoes_pagamento não existem neste schema
      ...
      const data = {
        ...
        alertas: [
          obrasAtivasCount > 10 
            ? { tipo: 'warning', mensagem: `AtenÃ§Ã£o: ${obrasAtivasCount} obras ativas simultaneamente.` }
            : { tipo: 'info', mensagem: `${obrasAtivasCount} obras em andamento.` },
          solicitacoesPendentesCount > 0
            ? { tipo: 'info', mensagem: `${solicitacoesPendentesCount} solicitaÃ§Ãµes de pagamento aguardando aprovaÃ§Ã£o.` }
            : { tipo: 'success', mensagem: 'Nenhuma solicitaÃ§Ã£o pendente.' }
        ]
      };
  ...
  return (
    <>
      <Helmet>
        <title>Dashboard - WG Almeida GestÃ£o Financeira</title>
        <meta name="description" content="VisÃ£o consolidada das finanÃ§as do Grupo WG Almeida" />
      </Helmet>

      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Dashboard Financeiro</h1>
          <p className="text-gray-600 mt-1">VisÃ£o consolidada do Grupo WG Almeida</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="wg-card p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 font-medium">Custos Totais (MÃªs)</p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="wg-card p-6 lg:col-span-3">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={dashboardData.fluxoMensal}>
                ...
                <Line type="monotone" dataKey="saida" stroke="#F25C26" strokeWidth={2} name="SaÃ­das" />
              </LineChart>
            </ResponsiveContainer>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="wg-card p-6 lg:col-span-2">
            <h2 className="text-lg font-bold text-gray-900 mb-4">DistribuiÃ§Ã£o de Custos</h2>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
