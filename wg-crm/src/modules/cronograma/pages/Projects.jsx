...
import { useEntities } from '@/modules/cronograma/hooks/useEntities';
...
const ProjectForm = ({ onFinished }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        onFinished();
        navigate(`${basePath}/projects/${newProject.id}`);
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProjects.map((project, index) => (
                <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="relative group">
                  <Link to={`${basePath}/projects/${project.id}`} className="block h-full">
