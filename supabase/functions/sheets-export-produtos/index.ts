...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: rows, error } = await supabase.from('produtos_servicos').select('*').order('nome');
    if (error) {
    ...
    const sheetId = Deno.env.get('SHEET_PRODUTOS_ID');
    if (!sheetId) {
      throw new Error("Missing SHEET_PRODUTOS_ID environment variable");
    }
    const range = 'Produtos!A1';
